...
internal protocol OnboardPageViewControllerDelegate: AnyObject {
    ...
    func pageViewController(_ pageVC: OnboardPageViewController, editedAt index: Int, text: String)
...
internal final class OnboardPageViewController: UIViewController {
  ...
  override func loadView() {
    ...
    textField.addTarget(self, action: #selector(OnboardPageViewController.textFieldDidChange(_:)), for: .editingChanged)
  ...
  func configureWithPage(_ page: OnboardPage) {
    ...
    configureTextField(page.textField)
  ...
  @objc fileprivate func advanceTapped() {
    ...
    @objc fileprivate func textFieldDidChange(_ sender: UITextField) {
        if (sender.text?.count)! > 0 {
            delegate?.pageViewController(self, editedAt: pageIndex, text: sender.text!)
        }
    }
}
